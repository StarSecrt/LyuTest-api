import { Injectable,Logger } from '@nestjs/common';
import { NotFoundException } from '@nestjs/common/exceptions';
import { LoggerService } from '@nestjs/common/services';
import { InjectRepository } from '@nestjs/typeorm';
import { T_BusinessPartnerEntity } from 'src/admin/bp/entity/T_business_partner.entity';
import { ApiService } from 'src/api/api.service';
import { DataSource, Repository } from 'typeorm';
import { CreatePosItemDto } from './dto/create-positem.dto';
import { T_InterfaceItemEntity } from './entity/T_interface_item.entity';

@Injectable()
export class InterfaceItemService {
    constructor(        
        @InjectRepository(T_InterfaceItemEntity) private T_InterfaceItemRepository: Repository<T_InterfaceItemEntity>,
        @InjectRepository(T_BusinessPartnerEntity) private T_BusinessPartnerRepository: Repository<T_BusinessPartnerEntity>,
        private dataSource: DataSource,
        private apiService: ApiService,
        private readonly logger: Logger,
    ) {}    
    //Log 호출
    private PrintLoggerServiceLog(logTitle: string, logData: any) {
        this.logger.log(`[Log] ` + logTitle + `: ` + JSON.stringify(logData));
    }

    /************************************  Loop Test Start ************************************/
    // LoopSample() {
    //     const plArr: any = [
    //         {itemid: 11, itemcode: 'SP-0172-11'},
    //         {itemid: 22, itemcode: 'SP-0172-22'},
    //         {itemid: 33, itemcode: 'SP-0172-33'},
    //         {itemid: 44, itemcode: 'SP-0172-44'}
    //     ];

    //     console.log(`For`)
    //     for (let Fidx = 0; Fidx < plArr.length; Fidx++) {
    //         const content: any = plArr[Fidx];
    //         console.log(plArr[Fidx]);
    //     }

    //     console.log(`While`)
    //     let Widx: number = 0;
    //     while(Widx < plArr.length) {
    //         const content: any = plArr[Widx];
    //         console.log(plArr[Widx]);
    //         Widx++;
    //     }
    // }
    /************************************  Loop Test End ************************************/

    /************************************  API Test Start ************************************/
    selectTestList(): Promise<T_BusinessPartnerEntity[]> {
        const selectQuery = `select * from public."T_BUSINESS_PARTNER" where c_code like 'P%'`;
        const resData = this.dataSource.query(selectQuery);
        return resData;
    }

    async selectTestList2(): Promise<T_BusinessPartnerEntity[]> {
        const resData = await this.dataSource.manager
            .createQueryBuilder()
            .select("T_BUSINESS_PARTNER")
            .from(T_BusinessPartnerEntity, "T_BUSINESS_PARTNER")
            .where(
                "c_code like :code",
                { code: 'P%' })
            .orderBy({
                "c_index": "DESC"
            })
            .getMany();        
        return resData;
    }

    selectTestFindOne(bpCode: string): Promise<object> {
        const resData = this.T_BusinessPartnerRepository.findOne({
            where: { c_code: bpCode }
        })
        return resData;
    }
    /************************************  API Test End ************************************/


    // Typeorm Function 사용
    async interfaceItemList(page: number): Promise<T_InterfaceItemEntity[]> {
        const resData = this.T_InterfaceItemRepository.find({
            where: { c_item_size: "S" },
            order: {
                c_item_id: "DESC",
                c_index: "DESC"
            },
            skip: (page -1) * 10,
            take: 10
        });
        return resData;
    }

    // Query 작성
    async interfaceItemList_query(page: number): Promise<T_InterfaceItemEntity[]> {
        const selectQuery = `select * from public."T_INTERFACE_ITEM" where c_index > 500 order by c_index desc`;
        const resData = this.dataSource.query(selectQuery);
        return resData;
    }
    
    // Query Builder 사용 
    async interfaceItemList_builder(page: number): Promise<T_InterfaceItemEntity[]> {
        const resData = await this.dataSource.manager
            // .createQueryBuilder(T_INTERFACE_ITEM, "T_INTERFACE_ITEM")
            .createQueryBuilder()
            .select("T_INTERFACE_ITEM")
            .from(T_InterfaceItemEntity, "T_INTERFACE_ITEM")
            .where(
                "c_index > :index and c_item_id > :item_id",
                { index: 400, item_id: 2200 })
            .andWhere("c_item_size = :item_size", { item_size: 'XL' })
            .orderBy({ "c_index": "DESC" })
            .skip((page -1) * 10)
            .take(10) 
            .getMany();        
        return resData;
    }

    /* POS 계정 정보 기준 도매 Item 연동 > 등록/수정 요청 */
    async getPosItemList(aspid: number, slrid: number, itemid: number): Promise<any> {
        let DateTime = new Date(+new Date() + 3240 * 10000).toISOString().replace("T"," ").replace(/\..*/,'');
        // apiservice 부분 수정 필요
        const data = await this.apiService.getPosItemList(aspid, slrid, itemid);
        const resData = JSON.stringify(data)
        const parseData = JSON.parse(resData);

        console.log(`Data save start`);        
        const resItemData = parseData.itemlist;        
        // console.log(resItemData);
        
        let Widx: number = 0;
        let CreateCount: number = 0;
        let UpdateCount: number = 0;
        let ErrorCount: number = 0;
        let saveResult: any;
        // loop start
        while(Widx < resItemData.length) {
            const content: any = resItemData[Widx];
            // console.log(resItemData[Widx]);
            saveResult = await this.savePosItem(aspid, slrid, resItemData[Widx]);
            // console.log(Widx + ` : ` + saveResult);

            if (saveResult == 'create') {
                CreateCount++;
            } else if (saveResult == 'update') {
                UpdateCount++;
            } else if (saveResult == 'error') {
                ErrorCount++;
            } 
            
            Widx++;
        }        
        console.log(`Data save end`);
        let returnMsg = {'DateTime': DateTime, 'Total Count': Widx, 'CreateCount': CreateCount, 'UpdateCount': UpdateCount, 'ErrorCount': ErrorCount};
        this.PrintLoggerServiceLog('Interface Item Save Result', returnMsg); 
        return returnMsg;
    }

     /* BP Code 기준 POS Item 연동 > 등록/수정 요청 */
     async getPosItemListBP(bpcode: string): Promise<any> {
        let DateTime = new Date(+new Date() + 3240 * 10000).toISOString().replace("T"," ").replace(/\..*/,'');
        let bpData = await this.T_BusinessPartnerRepository.findOne({
            where: { c_code: bpcode }
        });

        if (!bpData) {
            throw new NotFoundException(`Can't find with BP Code : ${bpcode}`);
        }

        let aspid = bpData.c_pos_id;
        let slrid = bpData.c_pos_shopid;
        let itemid = 0;

        if (!aspid) {
            throw new NotFoundException(`Can't find POS ID with BP Code : ${bpcode}`);
        }

        const data = await this.apiService.getPosItemList(aspid, slrid, itemid);
        const resData = JSON.stringify(data);
        const parseData = JSON.parse(resData);

        console.log(`Data save start`);        
        const resItemData = parseData.itemlist;        
        // console.log(resItemData);
        
        let Widx: number = 0;
        let CreateCount: number = 0;
        let UpdateCount: number = 0;
        let ErrorCount: number = 0;
        let saveResult: any;
        // loop start
        while(Widx < resItemData.length) {
            const content: any = resItemData[Widx];
            // console.log(resItemData[Widx]);
            saveResult = await this.savePosItem(aspid, slrid, resItemData[Widx]);
            // console.log(Widx + ` : ` + saveResult);

            if (saveResult == 'create') {
                CreateCount++;
            } else if (saveResult == 'update') {
                UpdateCount++;
            } else if (saveResult == 'error') {
                ErrorCount++;
            };
            Widx++;
        }        
        console.log(`Data save end`);
        let returnMsg = {'DateTime': DateTime, 'Total Count': Widx, 'CreateCount': CreateCount, 'UpdateCount': UpdateCount, 'ErrorCount': ErrorCount};
        this.PrintLoggerServiceLog('Interface Item Save Result', returnMsg);            
        return returnMsg;
    }

    /* POS Item 등록/수정 */
    private async savePosItem(aspid: number, slrid: number, itemData: CreatePosItemDto): Promise<any> {
        // console.log(`Data Save Start`);
        // console.log(itemData);
        const {
            itemid, itemcode, itemname, colorname, sizename, fiber, wholeprice, regidate, image0, image1,
            image2, image3, image4, eancode, barcode, lastqty
        } = itemData;
        // console.log(itemid);
        const posItemFind = await this.T_InterfaceItemRepository.findOne({
            where: { c_aspid: aspid, c_slrid: slrid, c_item_id: itemid}
        });
        
        try {
            if (!posItemFind) {
                await this.dataSource.transaction(async manager => {
                    const interfaceItem = new T_InterfaceItemEntity();

                    interfaceItem.c_aspid = aspid;
                    interfaceItem.c_slrid = slrid;
                    interfaceItem.c_item_id = itemid;
                    interfaceItem.c_item_code = itemcode;
                    interfaceItem.c_item_name = itemname;
                    interfaceItem.c_reference_color = colorname;
                    interfaceItem.c_item_size = sizename;
                    interfaceItem.c_mixing_ratio = fiber;
                    interfaceItem.c_supply_price = wholeprice;
                    interfaceItem.c_item_reg_date = regidate;
                    interfaceItem.c_item_image1 = image0;
                    interfaceItem.c_item_image2 = image1;
                    interfaceItem.c_item_image3 = image2;
                    interfaceItem.c_item_image4 = image3;
                    interfaceItem.c_item_image5 = image4;
                    interfaceItem.c_internal_barcode = eancode;
                    interfaceItem.c_external_barcode = barcode;
                    interfaceItem.c_available_stock = lastqty;
                    interfaceItem.c_status = 'I';
                    interfaceItem.c_create_user_index = 0;
                    interfaceItem.c_create_date = new Date();
                    interfaceItem.c_update_user_index = 0;
                    interfaceItem.c_update_date = new Date();
                    
                    // console.log(`Create Interface Item`);
                    await manager.save(interfaceItem);
                });
            } else {
                await this.dataSource.transaction(async manager => {
                    posItemFind.c_item_id = itemid;
                    posItemFind.c_item_code = itemcode;
                    posItemFind.c_item_name = itemname;
                    posItemFind.c_reference_color = colorname;
                    posItemFind.c_item_size = sizename;
                    posItemFind.c_mixing_ratio = fiber;
                    posItemFind.c_supply_price = wholeprice;
                    posItemFind.c_item_reg_date = regidate;
                    posItemFind.c_item_image1 = image0;
                    posItemFind.c_item_image2 = image1;
                    posItemFind.c_item_image3 = image2;
                    posItemFind.c_item_image4 = image3;
                    posItemFind.c_item_image5 = image4;
                    posItemFind.c_internal_barcode = eancode;
                    posItemFind.c_external_barcode = barcode;
                    posItemFind.c_available_stock = lastqty;
                    posItemFind.c_status = 'U';
                    posItemFind.c_update_user_index = 0;
                    posItemFind.c_update_date = new Date();
                    
                    // console.log(`Update Interface Item`);
                    // this.PrintLoggerServiceLog(itemid, 'Interface Item Update');
                    await manager.save(posItemFind);
                });
            };

            if (!posItemFind) {
                return `create`;
            } else {
                return `update`;
            };
        } catch (error) {
            this.PrintLoggerServiceLog('Interface Item Save Error', itemid);            
            // console.error (error);            
            return `error`;
        }           
    }

    

}
