...
let AuthService = class AuthService {
    ...
    createToken(payload, secretKey, tokenTime) {
        return jwt.sign({
            index: payload.index,
            name: payload.name,
            email: payload.email
    ...
    verify(jwtString) {
        try {
            const payload = jwt.verify(jwtString, this.config.jwtAccessSecret);
            ...
            throw new common_1.UnauthorizedException('승인에 실패하였습니다.');
        }
    }
    refreshVerify(jwtString) {
        try {
            const payload = jwt.verify(jwtString, this.config.jwtRefreshSecret);
            const { id, email } = payload;
            return {
                userId: id,
                email,
            ...
            throw new common_1.UnauthorizedException('Refresh Token 승인에 실패하였습니다.');
        }
    }
};
